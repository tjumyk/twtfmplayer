...
public class FxExperiencePlayer extends Application {
	...
	public void start(final Stage primaryStage) {
		...
		ImageView background = new ImageView(
				new Image(
						FxExperiencePlayer.class
								.getResourceAsStream("images/FXExperiencePlayer-BG.png")));

		for (int i = 0; i < 10; i++) {
			final int fi = i;
			sliders[i] = new Slider(EqualizerBand.MIN_GAIN,
					EqualizerBand.MAX_GAIN, 0);
		...
		trackLabel = new Label("No Track");
		timeLabel = new Label("");
		nameLabel = new Label("");
		Font lcd = Font.loadFont(
				FxExperiencePlayer.class.getResourceAsStream("lcddot.ttf"), 14);
		...
		Line leftVUPointer = LineBuilder
				.create()
				.startX(0)
				.startY(0)
				.endX(0)
				.endY(-83)
				.stroke(new LinearGradient(0, 1, 0, 0, true,
						CycleMethod.NO_CYCLE,
						new Stop(0.33, Color.TRANSPARENT), new Stop(0.34,
								Color.BLACK), new Stop(1, Color.web("#7e7e7e"))))
				.effect(DropShadowBuilder.create().radius(5)
						.color(new Color(0, 0, 0, 0.3)).build()).strokeWidth(3)
				.translateX(375).translateY(615).transforms(leftVURotation)
				.build();
		...
		Line rightVUPointer = LineBuilder
				.create()
				.startX(0)
				.startY(0)
				.endX(0)
				...
				.effect(DropShadowBuilder.create().radius(5)
						.color(new Color(0, 0, 0, 0.3)).build()).strokeWidth(3)
				.translateX(593).translateY(615).transforms(rightVURotation)
				.build();
		for (int i = 0; i < 10; i++)
		...
		root.getChildren().addAll(background, prevBtn, playPauseBtn, nextBtn,
				loadBtn, powerBtn, trackLabel, timeLabel, nameLabel,
				leftVUPointer, rightVUPointer, balanceKnob, volumeKnob);
		...
		scene.getStylesheets().add(
				FxExperiencePlayer.class.getResource("FxExperiencePlayer.css")
						.toExternalForm());
		primaryStage.setScene(scene);
		primaryStage.initStyle(StageStyle.TRANSPARENT);
		primaryStage.show();

		// Use ScenicView to debug
		// ScenicView.show(scene);

		// listen for when we have songs
		playList.getSongs().addListener(
				new ListChangeListener<Pair<String, String>>() {
					@Override
					public void onChanged(
							Change<? extends Pair<String, String>> arg0) {
	...
	private void play(int songIndex) {
		...
		final Media media = new Media(playList.getSongs().get(songIndex)
				.getValue());
		...
		trackLabel.setText("Track " + (curreentSongIndex + 1) + "/"
				+ playList.getSongs().size());
		nameLabel.setText(playList.getSongs().get(curreentSongIndex).getKey());
		timeLabelValue.bind(new StringBinding() {
			{
				bind(mediaPlayer.currentTimeProperty(),
						media.durationProperty(), mediaPlayer.statusProperty());
			}
			...
			protected String computeValue() {
				...
				case PLAYING:
					if (media == null || media.getDuration() == null) {
						...
						return "Time: 00:00   Remaining: 00:00   Total: "
								+ formatDuration(media.getDuration());
					} else {
						return "Time: "
								+ formatDuration(mediaPlayer.getCurrentTime())
								+ "   Remaining: "
								+ formatDuration(media.getDuration().subtract(
			...
			String formatDuration(Duration time) {
				...
				int secondsWhole = (int) Math
						.round((minutes - minutesWhole) * 60);
				return String.format("%1$02d:%2$02d", minutesWhole,
						secondsWhole);
			}
		});
